...
class MongoController{
    ...
    async setBinanceWalletAddress(addressObject) {
        let newWallet = new this.BinanceWallet(addressObject);
    ...
    async setBlockNumberTimeStamp(blockNumberObject) {
        let newEntry = new this.BlockNumberTimeStamp(blockNumberObject);
    ...
    async setBinanceTokenTransfer(binanceTransferObject) {
        let newEntry = new this.BinanceTokenTransfer(binanceTransferObject);
    ...
    async isBinanceWalletStored(addressToCheck){
        let result = await this.BinanceWallet.find({_id: addressToCheck});
    ...
    async isBlockNumberStored(blockNumber){
        let result = await this.BlockNumberTimeStamp.find({_id: blockNumber});
    ...
    async isBinanceTransferStored(transactionHash){
        let result = await this.BinanceTokenTransfer.find({_id: transactionHash});
    ...
    async deleteBinanceWalletAddress(addressToDelete){
       return await this.BinanceWallet.deleteOne({_id: addressToDelete});
    }
    ...
    async deleteBlocknumber(blockNumberToDelete){
        return await this.BlockNumberTimeStamp.deleteOne({_id: blockNumberToDelete});
    }
    ...
    /// Throws an exception if there is an error checking the database.
    /// Param: String(transactionHash = "0xdcf194aed18214b430918a1a0934840ec196230b71f0977a3613614bcf7cac9c")
    async deleteTransferEvent(blockNumberToDelete){
        return await this.BinanceTokenTransfer.deleteOne({_id: blockNumberToDelete});
    }


    //--------------Get Entire Database Metods
